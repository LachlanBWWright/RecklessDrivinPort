...
void DrawTextureBlock(int x,int y,int size,UInt32 zoom,UInt32 u,UInt32 v,UInt8 *texture)
{
	int line,pix;
...
void DrawTextureBlockClipped(int x,int y,int size,UInt32 zoom,UInt32 u,UInt32 v,UInt8 *texture)
{
	int line,pix;
	...
	if(x<0){xSize+=x;x=0;}
	if(y<0){ySize+=y;y=0;}
	if(x+size>gXSize)xSize+=gXSize-x-size;
	if(y+size>gYSize)ySize+=gYSize-y-size;
			...
			if((x1>-size||x2>-size)&&(x1<gXSize||x2<gXSize)){	
				...
				if(dy){
					...
					for(y=y1;y<y2;y++)
					{
						...
						{
							if(blockX>=0&&blockX<gXSize-size&&y>=0&&y<gYSize-size)
								DrawTextureBlock(blockX,y,size,fixZoom,blockU,v,texture);
							else 
								DrawTextureBlockClipped(blockX,y,size,fixZoom,blockU,v,texture);
							blockU+=fixZoom;
							blockX++;
						}
						x+=dxdy;
					...
					int x;
					if(x2<0)x2=0;
					if(x1>gXSize-size)x1=gXSize-size;
					if(x1<x2)
					{						
						SInt32 u=u1;
						if(x1<0)x1=0;
						if(x2>gXSize-size)x2=gXSize-size;
						for(x=x1;x<x2;x++)
							DrawTextureBlockClipped(x,y1,size,fixZoom,u+=fixZoom,v1,texture);
					}
					else{
						SInt32 u=u2;
						if(x2<0)x2=0;
...
void DrawTextureBlock16(UInt32 x,UInt32 y,int size,UInt32 zoom,UInt32 u,UInt32 v,UInt16 *texture)
{
	int line,pix;
...
void DrawTextureBlockClipped16(int x,int y,int size,UInt32 zoom,UInt32 u,UInt32 v,UInt16 *texture)
{
	int line,pix;
	int xSize=size,ySize=size;
	UInt16 *dst;
			...
			if((x1>-size||x2>-size)&&(x1<gXSize||x2<gXSize)){	
				...
				if(dy){
					...
					for(y=y1;y<y2;y++)
					{
						...
						{
							if(blockX>=0&&blockX<gXSize-size&&y>=0&&y<gYSize-size)
								DrawTextureBlock16(blockX,y,size,fixZoom,blockU,v,texture);
							else 
								DrawTextureBlockClipped16(blockX,y,size,fixZoom,blockU,v,texture);
							blockU+=fixZoom;
							blockX++;
						}
						x+=dxdy;
					...
					{						
						...
						if(x2>gXSize-size)x2=gXSize-size;
						for(x=x1;x<x2;x++)
							DrawTextureBlockClipped16(x,y1,size,fixZoom,u+=fixZoom,v1,texture);
					}
					else{
						SInt32 u=u2;
						if(x2<0)x2=0;
