...
#define kVeloSteerTime		0.075	//multiplied by cars velocity and added to kMinSteerTime.
#define kJustTime			0.2		//minimum time (in seconds) for the steering wheel to recenter.
...
#define kSlideFactor		0.5	//determines how much control one has over the car when it is sliding (0=no difference/1= 'no control')
...
t2DPoint CalcWheelForce(float dir,float wheelPower,float fRoadFriction,float mass,float velo,t2DPoint vVelo,tObject *theObj,float *slide)
{
	...
	if(theObj->input.brake&&!(*gRoadInfo).water&&velo)
	{
		if(theObj->input.brake<0.8)
			fWheel=VEC2D_Scale(vVelo,-1500*mass*theObj->input.brake*theObj->input.brake/velo);
		else
	...
	*slide+=slideOut/4;
	...
	fResistance=VEC2D_Sum(VEC2D_Scale(theObj->velo,-velo*(*gRoadInfo).airResistance),VEC2D_Scale(vVelo,-kMechResistance));
	fWheel=VEC2D_Sum(fWheel,fResistance);
	return fWheel;
}

t2DPoint CalcWheelForcePlayer(float dir,float wheelPower,float fRoadFriction,float mass,float velo,t2DPoint vVelo,tObject *theObj,float *slide,float brake)
{
...
void AddTrack(t2DPoint p1,t2DPoint p2, float intensity)
{
	...
	{
		BlockMoveData(gTracks+kMaxTracks/2,gTracks,kMaxTracks/2*sizeof(tTrackSeg));
...
void CalcSteeringPlayer(tObject *theObj,float velo)
{
	...
	{
		theObj->steering+=kFrameDuration/(theObj->steering>0?steerTime:kJustTime);
		if(theObj->input.steering*maxSteering<theObj->steering)
...
float CalcPower(tObject *theObj,float velo)
{
	...
	float signedVelo=VEC2D_DotProduct(theObj->velo,P2D(sin(theObj->dir),cos(theObj->dir)));
	float power;
	int accel=((theObj->input.kickdown&&!gRoadInfo->water)?(velo<25||signedVelo<0?50:3):2);
	float throttle=theObj->throttle*(velo<50?accel/(velo*0.1+1)+1-accel/6:1)*15;
	...
	if(theObj==gPlayerObj)
		power*=(gPlayerAddOns&kAddOnTurbo&&gRoadInfo->friction>=100?1.7:1);
...
void ControlWheelObjectPlayer(tObject *theObj)
{
	...
	float fRoadFriction=(*objType).mass*(*objType).friction*(*gRoadInfo).friction;
	float power=CalcPower(theObj,velo);
	float rotMass=(*objType).length*(*objType).length*(*objType).mass;
	float slide[4]={0,0,0,0};
	
	t2DPoint pFrontLeft=P2D(sinDir*(*objType).wheelLength-cosDir*(*objType).wheelWidth,cosDir*(*objType).wheelLength+sinDir*(*objType).wheelWidth);
	t2DPoint vFrontLeft=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pFrontLeft),theObj->rotVelo));
	t2DPoint pTotalFrontLeft=VEC2D_Sum(VEC2D_Scale(pFrontLeft,kScale),theObj->pos);
	t2DPoint fFrontLeft=CalcWheelForcePlayer(theObj->dir+((*objType).flags2&kObjectRearSteer?0:theObj->steering)
			,((*objType).flags2&kObjectRearDrive?0:power)
			,fRoadFriction*SlideFriction(gPlayerSlide[0]),(*objType).mass,velo,vFrontLeft,theObj,&slide[0],theObj->input.brake);
	float mFrontLeft=VEC2D_CrossProduct(fFrontLeft,pFrontLeft);

	t2DPoint pFrontRight=P2D(sinDir*(*objType).wheelLength+cosDir*(*objType).wheelWidth,cosDir*(*objType).wheelLength-sinDir*(*objType).wheelWidth);
	t2DPoint vFrontRight=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pFrontRight),theObj->rotVelo));
	t2DPoint pTotalFrontRight=VEC2D_Sum(VEC2D_Scale(pFrontRight,kScale),theObj->pos);
	t2DPoint fFrontRight=CalcWheelForcePlayer(theObj->dir+((*objType).flags2&kObjectRearSteer?0:theObj->steering)
			,((*objType).flags2&kObjectRearDrive?0:power)
			,fRoadFriction*SlideFriction(gPlayerSlide[1]),(*objType).mass,velo,vFrontRight,theObj,&slide[1],theObj->input.brake);
	float mFrontRight=VEC2D_CrossProduct(fFrontRight,pFrontRight);

	t2DPoint pRearLeft=P2D(-sinDir*(*objType).wheelLength-cosDir*(*objType).wheelWidth,-cosDir*(*objType).wheelLength+sinDir*(*objType).wheelWidth);
	t2DPoint vRearLeft=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pRearLeft),theObj->rotVelo));
	t2DPoint pTotalRearLeft=VEC2D_Sum(VEC2D_Scale(pRearLeft,kScale),theObj->pos);
	t2DPoint fRearLeft=CalcWheelForcePlayer(theObj->dir-((*objType).flags2&kObjectRearSteer?theObj->steering:0)
			,((*objType).flags2&kObjectRearDrive?power:0)
			,fRoadFriction*SlideFriction(gPlayerSlide[2]),(*objType).mass,velo,vRearLeft,theObj,&slide[2],theObj->input.handbrake+0.8*theObj->input.brake);
	float mRearLeft=VEC2D_CrossProduct(fRearLeft,pRearLeft);

	t2DPoint pRearRight=P2D(-sinDir*(*objType).wheelLength+cosDir*(*objType).wheelWidth,-cosDir*(*objType).wheelLength-sinDir*(*objType).wheelWidth);
	t2DPoint vRearRight=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pRearRight),theObj->rotVelo));
	t2DPoint pTotalRearRight=VEC2D_Sum(VEC2D_Scale(pRearRight,kScale),theObj->pos);
	t2DPoint fRearRight=CalcWheelForcePlayer(theObj->dir-((*objType).flags2&kObjectRearSteer?theObj->steering:0)
			,((*objType).flags2&kObjectRearDrive?power:0)
			,fRoadFriction*SlideFriction(gPlayerSlide[3]),(*objType).mass,velo,vRearRight,theObj,&slide[3],theObj->input.handbrake+0.8*theObj->input.brake);
	float mRearRight=VEC2D_CrossProduct(fRearRight,pRearRight);
	
	t2DPoint aTotal=VEC2D_Scale(VEC2D_Sum(VEC2D_Sum(VEC2D_Sum(fFrontLeft,fFrontRight),fRearLeft),fRearRight),0.25/(*objType).mass*kFrameDuration);
	float aRotTotal=(mFrontLeft+mFrontRight+mRearLeft+mRearRight)/(4*rotMass)*kFrameDuration;

	if(slide[0]>(*gRoadInfo).trackSlide)
			if(!CalcBackCollision(pTotalFrontLeft))
				AddTrack(pTotalFrontLeft,VEC2D_Sum(VEC2D_Scale(vFrontLeft,kScale*kFrameDuration),pTotalFrontLeft),(slide[0]-(*gRoadInfo).trackSlide)*kTrackIntensity);
	if(slide[0]>(*gRoadInfo).dustSlide)
			NewParticleFX(pTotalFrontLeft,vFrontLeft,12,(*gRoadInfo).dustColor,0,20);
	if(slide[1]>(*gRoadInfo).trackSlide)
			if(!CalcBackCollision(pTotalFrontRight))
				AddTrack(pTotalFrontRight,VEC2D_Sum(VEC2D_Scale(vFrontRight,kScale*kFrameDuration),pTotalFrontRight),(slide[1]-(*gRoadInfo).trackSlide)*kTrackIntensity);
	if(slide[1]>(*gRoadInfo).dustSlide)
			NewParticleFX(pTotalFrontRight,vFrontRight,12,(*gRoadInfo).dustColor,0,20);
	if(slide[2]>(*gRoadInfo).trackSlide)
			if(!CalcBackCollision(pTotalRearLeft))
				AddTrack(pTotalRearLeft,VEC2D_Sum(VEC2D_Scale(vRearLeft,kScale*kFrameDuration),pTotalRearLeft),(slide[2]-(*gRoadInfo).trackSlide)*kTrackIntensity);
	...
	if(slide[3]>(*gRoadInfo).trackSlide)
			if(!CalcBackCollision(pTotalRearRight))
				AddTrack(pTotalRearRight,VEC2D_Sum(VEC2D_Scale(vRearRight,kScale*kFrameDuration),pTotalRearRight),(slide[3]-(*gRoadInfo).trackSlide)*kTrackIntensity);
	if(slide[3]>(*gRoadInfo).dustSlide)
			NewParticleFX(pTotalRearRight,vRearRight,12,(*gRoadInfo).dustColor,0,20);
...
void ControlWheelObject(tObject *theObj)
{
	...
	t2DPoint objDir=P2D(sinDir,cosDir);
	float velo=VEC2D_Value(theObj->velo);
	float fRoadFriction=(*objType).mass*(*objType).friction*(*gRoadInfo).friction*SlideFriction(theObj->slide);
	float power=CalcPower(theObj,velo);
	float rotMass=(*objType).length*(*objType).length*(*objType).mass;
	float slide=0;
	...
	t2DPoint pFrontLeft=P2D(sinDir*(*objType).wheelLength-cosDir*(*objType).wheelWidth,cosDir*(*objType).wheelLength+sinDir*(*objType).wheelWidth);
	t2DPoint vFrontLeft=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pFrontLeft),theObj->rotVelo));
	t2DPoint fFrontLeft=CalcWheelForce(theObj->dir+theObj->steering,power,fRoadFriction,(*objType).mass,velo,vFrontLeft,theObj,&slide);
	float mFrontLeft=VEC2D_CrossProduct(fFrontLeft,pFrontLeft);

	t2DPoint pFrontRight=P2D(sinDir*(*objType).wheelLength+cosDir*(*objType).wheelWidth,cosDir*(*objType).wheelLength-sinDir*(*objType).wheelWidth);
	t2DPoint vFrontRight=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pFrontRight),theObj->rotVelo));
	t2DPoint fFrontRight=CalcWheelForce(theObj->dir+theObj->steering,power,fRoadFriction,(*objType).mass,velo,vFrontRight,theObj,&slide);
	float mFrontRight=VEC2D_CrossProduct(fFrontRight,pFrontRight);

	t2DPoint pRearLeft=P2D(-sinDir*(*objType).wheelLength-cosDir*(*objType).wheelWidth,-cosDir*(*objType).wheelLength+sinDir*(*objType).wheelWidth);
	t2DPoint vRearLeft=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pRearLeft),theObj->rotVelo));
	t2DPoint fRearLeft=CalcWheelForce(theObj->dir,0,fRoadFriction,(*objType).mass,velo,vRearLeft,theObj,&slide);
	float mRearLeft=VEC2D_CrossProduct(fRearLeft,pRearLeft);

	t2DPoint pRearRight=P2D(-sinDir*(*objType).wheelLength+cosDir*(*objType).wheelWidth,-cosDir*(*objType).wheelLength-sinDir*(*objType).wheelWidth);
	t2DPoint vRearRight=VEC2D_Sum(theObj->velo,VEC2D_Scale(VEC2D_CP(pRearRight),theObj->rotVelo));
	t2DPoint fRearRight=CalcWheelForce(theObj->dir,0,fRoadFriction,(*objType).mass,velo,vRearRight,theObj,&slide);
	float mRearRight=VEC2D_CrossProduct(fRearRight,pRearRight);
	
	t2DPoint aTotal=VEC2D_Scale(VEC2D_Sum(VEC2D_Sum(VEC2D_Sum(fFrontLeft,fFrontRight),fRearLeft),fRearRight),0.25/(*objType).mass*kLowFrameDuration);
	float aRotTotal=(mFrontLeft+mFrontRight+mRearLeft+mRearRight)/(4*rotMass)*kLowFrameDuration;
...
void ControlHeliObject(tObject *theObj)
{
	...
	float spanArea=fabs(VEC2D_DotProduct(objDir,VEC2D_Norm(theObj->velo)))*3+fabs(VEC2D_DotProduct(VEC2D_CP(objDir),VEC2D_Norm(theObj->velo))*10);
	t2DPoint vfEngine=VEC2D_Scale(objDir,fEngine);
	t2DPoint vfResistance=VEC2D_Scale(theObj->velo,-velo*(*gRoadInfo).airResistance*spanArea);	
	t2DPoint aTotal=VEC2D_Scale(VEC2D_Sum(vfResistance,vfEngine),kLowFrameDuration/(*objType).mass);
	theObj->velo=VEC2D_Sum(aTotal,theObj->velo);
	...
	if(fabs(theObj->rotVelo)>kMaxHeliRotVelo)
		theObj->rotVelo=(theObj->rotVelo>0?kMaxHeliRotVelo:-kMaxHeliRotVelo);
	theObj->steering=theObj->input.steering;
}
...
{
	tObjectTypePtr objType=theObj->type;
	float aFriction=0.015*(*objType).friction*(*gRoadInfo).friction*kLowFrameDuration;
	float velo=VEC2D_Value(theObj->velo);	
	if(aFriction<velo)
	{
	...
	if(theObj->rotVelo)
	{
		theObj->rotVelo=theObj->rotVelo+(theObj->rotVelo>0?-aFriction:aFriction);
		...
		else if(fabs(theObj->pos.y-gCameraObj->pos.y)<kVisDist)
		{
			...
			if(theObj->type->flags&kObjectCop)
				if(!(gPlayerAddOns&kAddOnCop)&&!gFinishDelay&&!gPlayerDeathDelay)
				{
					...
					t2DPoint normDiffVec=VEC2D_Norm(VEC2D_Difference(gPlayerObj->pos,theObj->pos));
					if(theObj->control==kObjectDriveUp||theObj->control==kObjectNoInput)
					...
					else if(theObj->control==kObjectDriveDown)
					{
						...
						if(theObj->type->weaponObj)
							if(VEC2D_DotProduct(dirVec,normDiffVec)>0.9)
								if(VEC2D_Value(VEC2D_Difference(gPlayerObj->pos,theObj->pos))>70)
									if(RanProb(1.0/theObj->type->weaponInfo))
										FireWeapon(theObj,theObj->type->weaponObj);
				}
			if(theObj->type->flags2&kObjectDamageble){
				if(theObj->damage*2>theObj->type->maxDamage&&fabs(theObj->pos.y-gCameraObj->pos.y)<kVisDist)
					if(RanProb(0.25))
						MakeSmoke(theObj);
