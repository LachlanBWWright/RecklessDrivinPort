...
int CalcObjPoints(tObject *theObj,t2DPoint *points)
{
	...
	points[0]=P2D( sinDir*(*objType).length-cosDir*(*objType).width, cosDir*(*objType).length+sinDir*(*objType).width);
	points[1]=P2D( sinDir*(*objType).length+cosDir*(*objType).width, cosDir*(*objType).length-sinDir*(*objType).width);
	points[2]=P2D(-sinDir*(*objType).length+cosDir*(*objType).width,-cosDir*(*objType).length-sinDir*(*objType).width);
	points[3]=P2D(-sinDir*(*objType).length-cosDir*(*objType).width,-cosDir*(*objType).length+sinDir*(*objType).width);
	for(i=0;i<4;i++)
	{
...
int SectLines(t2DPoint *l1p1,t2DPoint *l1p2,t2DPoint *l2p1,t2DPoint *l2p2)
{
	...
	{
		float sectX=l2p1->x+(l2p2->x-l2p1->x)*l2p1offs/(l2p1offs-l2p2offs);
...
t2DPoint SectPoint(t2DPoint *l1p1,t2DPoint *l1p2,t2DPoint *l2p1,t2DPoint *l2p2)
{
...
int TestCollision(tObject *obj1,tObject *obj2,float sqDist)
{
	...
	if(obj2->jumpHeight!=obj1->jumpHeight&&!(((*obj1->type).flags2|(*obj2->type).flags2)&kObjectMissile)) return false;
	if(sqDist<sqr(kScale*(obj1->type->width<obj1->type->length?obj1->type->width:obj1->type->length)+(obj2->type->width<obj2->type->length?obj2->type->width:obj2->type->length)))
		return true;
	if(obj1Pts[hi1].y<obj2Pts[(hi2+2)&3].y) return false;
	if(obj2Pts[hi2].y<obj1Pts[(hi1+2)&3].y) return false;
	if(obj1Pts[(hi1+1)&3].x<obj2Pts[(hi2-1)&3].x) return false;
	if(obj2Pts[(hi2+1)&3].x<obj1Pts[(hi1-1)&3].x) return false;
		for(i=0;i<4;i++)
			for(j=0;j<4;j++)
				if(SectLines(&obj1Pts[i],&obj1Pts[(i+1)&3],&obj2Pts[j],&obj2Pts[(j+1)&3]))
					return true;
	if(SectLines(&obj1Pts[0],&obj1Pts[2],&obj2Pts[0],&obj2Pts[2])) return true;
	if(SectLines(&obj1Pts[1],&obj1Pts[3],&obj2Pts[0],&obj2Pts[2])) return true;
	if(SectLines(&obj1Pts[0],&obj1Pts[2],&obj2Pts[1],&obj2Pts[3])) return true;
	if(SectLines(&obj1Pts[1],&obj1Pts[3],&obj2Pts[1],&obj2Pts[3])) return true;
	return false;
}
...
void DamageObj(tObject *theObj,float damage,t2DPoint diff)
{
	...
	else if(damDir> 0.35) damagePos=VEC2D_CrossProduct(diff,P2D(sin(theObj->dir),cos(theObj->dir)))>0?kFrontRightTire:kFrontLeftTire;
	else if(damDir>-0.35) damagePos=VEC2D_CrossProduct(diff,P2D(sin(theObj->dir),cos(theObj->dir)))>0?kRightDoor:kLeftDoor;
	else if(damDir>-0.83) damagePos=VEC2D_CrossProduct(diff,P2D(sin(theObj->dir),cos(theObj->dir)))>0?kBackRightTire:kBackLeftTire;
	else damagePos=kBackBumper;
	Explosion(damPoint,theObj->velo,0,damage/maxDamage*2000,false);
	if(!(theObj->damageFlags&damagePos))
	{
		if(damagePos==kMotor)
			{if(theObj->damage>maxDamage*0.75) theObj->damageFlags|=damagePos;else return;}
		else
		...
		{					
			case kFrontBumper:
			case kMotor:
				theObj->frame=YDistortSprite(theObj->frame,-xSize,xSize,-ySize-1,-ySize+10,0,damage/maxDamage*20);
				break;	
			case kBackBumper:
				theObj->frame=YDistortSprite(theObj->frame,-xSize,xSize,ySize-10,ySize+1,1,damage/maxDamage*20);
				break;	
			case kFrontLeftTire:
				theObj->frame=XDistortSprite(theObj->frame,-ySize,-ySize*0.3,-xSize-1,-xSize+10,0,damage/maxDamage*20);
				break;	
			case kFrontRightTire:				
				theObj->frame=XDistortSprite(theObj->frame,-ySize,-ySize*0.3,xSize-10,xSize+1,1,damage/maxDamage*20);
				break;	
			case kBackLeftTire:				
				theObj->frame=XDistortSprite(theObj->frame,ySize*0.3,ySize,-xSize-1,-xSize+10,0,damage/maxDamage*20);
				break;	
			case kBackRightTire:				
				theObj->frame=XDistortSprite(theObj->frame,ySize*0.3,ySize,xSize-10,xSize+1,1,damage/maxDamage*20);
				break;	
			case kLeftDoor:
				theObj->frame=XDistortSprite(theObj->frame,-ySize*0.4,ySize*0.4,-xSize-1,-xSize+10,0,damage/maxDamage*20);
				break;	
			case kRightDoor:
				theObj->frame=XDistortSprite(theObj->frame,-ySize*0.4,ySize*0.4,xSize-10,xSize+1,1,damage/maxDamage*20);
				break;						
		}
...
void DoBounce(tObject *obj1,tObject *obj2,t2DPoint diff)
{
	...
	new12Velo=VEC2D_Sum(VEC2D_Scale(obj1->velo,mass1),VEC2D_Scale(obj2->velo,mass2));
	...
	if(obj1==gPlayerObj||obj2==gPlayerObj)
		...
		{
			if(fabs(VEC2D_DotProduct(diff,P2D(sin(gPlayerObj->dir),cos(gPlayerObj->dir))))<0.3)
				if(obj1==gPlayerObj)
	...
	if(obj1==gPlayerObj||obj2==gPlayerObj)
	{
		float joltMagnitude=(obj1==gPlayerObj)?VEC2D_Value(impulse1):VEC2D_Value(impulse2);
		joltMagnitude*=0.05;
		if(joltMagnitude>1)joltMagnitude=1;
...
void ShotHitObject(tObject *theObj,t2DPoint *srcPoint,t2DPoint *shotPoint,t2DPoint *impactPoint)
{
	...
	for(i=0;i<4;i++)
		...
		{
			t2DPoint sect=SectPoint(&objPts[i],&objPts[(i+1)&3],srcPoint,shotPoint);
	...
	*impactPoint=closeSectPoint;
	...
	localImpactPoint=P2D(sin(-theObj->dir)*localImpactPoint.y+cos(-theObj->dir)*localImpactPoint.x,-sin(-theObj->dir)*localImpactPoint.x+cos(-theObj->dir)*localImpactPoint.y);
	localImpactPoint=VEC2D_Scale(localImpactPoint,RanFl(0.5,1));
	theObj->frame=BulletHitSprite(theObj->frame,localImpactPoint.x,localImpactPoint.y);
...
void HandleShot(float length,tObject* shooter,t2DPoint offset)
{
	...
	for(i=0;i<kNumShots;i++)
	{
		...
		t2DPoint srcPoint=VEC2D_Sum(shooter->pos,P2D(sin(ranDir)*offset.y+cos(ranDir)*offset.x,-sin(ranDir)*offset.x+cos(ranDir)*offset.y));
		t2DPoint shotPoint=VEC2D_Sum(srcPoint,P2D(sin(ranDir)*ranLength,cos(ranDir)*ranLength));
		...
		while(theObj!=gLastVisObj)
		{
			...
			{
				t2DPoint relObjPos=VEC2D_Difference(theObj->pos,shooter->pos);
				float dist=fabs(VEC2D_DotProduct(normDir,relObjPos));
				if(dist<kMaxCollDist*2)
				{
					t2DPoint objPts[4];
					int i;
					CalcObjPoints(theObj,objPts);
		 			for(i=0;i<4;i++)
		 				if(SectLines(&objPts[i],&objPts[(i+1)&3],&srcPoint,&shotPoint))
		 				{
		 					t2DPoint objDist=VEC2D_Difference(theObj->pos,srcPoint);
		 					float sqDist=objDist.x*objDist.x+objDist.y*objDist.y;
		 					if(sqDist<closeDist)
		 					{
		 						closeDist=sqDist;
		 						closeObj=theObj;
		...
		{
			tObject *gunObj=NewObject(shooter,(CalcBackCollision(shotPoint)==2?(*gRoadInfo).deathOffs:0)+196);
