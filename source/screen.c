...
void InitScreen(int unused)
{
	...
	inDesiredAttributes.backBufferDepthMask=gPrefs.hiColor?kDSpDepthMask_16:kDSpDepthMask_8;
	inDesiredAttributes.displayDepthMask=gPrefs.hiColor?kDSpDepthMask_16:kDSpDepthMask_8;
...
void FadeScreen(int out)
{
	...
	switch(out)
	{
		case 1:DoError(DSpContext_FadeGammaOut(gDrawContext,0));break;
		case 0:DoError(DSpContext_FadeGammaIn(gDrawContext,0));break;
	}	
	if(out>=256)
     ...
     UInt32 destOdd)
{
    ...
    if (destOdd)
     ...
     srcMap.rowBytes = (0x3FFF & (srcMap.rowBytes << 1)) |
         (0xC000 & srcMap.rowBytes);

     //  double destination rowBytes (keep top two flag bits)
     destMap.rowBytes = (0x3FFF & (destMap.rowBytes << 1)) |
         (0xC000 & destMap.rowBytes);

     CopyBits ((BitMap *) &srcMap, (BitMap *) &destMap,
...
void Blit2Screen()
{
	...
	if(gScreenBlitSpecial)
	{
		...
		else if(gOSX)
		{
			DoError(DSpContext_GetBackBuffer(gDrawContext,kDSpBufferKind_Normal,&gameGW));
			DoError(DSpContext_GetFrontBuffer(gDrawContext,&screenGW));
			CopyBits(GetPortBitMapForCopyBits(gameGW),GetPortBitMapForCopyBits(screenGW),&rec,&rec,nil,srcCopy);
			SetGWorld(gameGW,nil);	
		}
		else{
