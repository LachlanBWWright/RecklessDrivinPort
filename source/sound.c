...
#define kMaxPanDist		400.0	//distance for maximum stereo panning (complete left or complete right)
#define kMaxListenDist	1250.0	//maximum distance from whihc sounds can be heard
#define kDopplerFactor	0.004	//determinates strenght of doppler effect simulation: 0 => no doppler effect
...
#define kHighestGearTurbo	70.0	//velocitiy at which highest gear is set with Turbo Engine(in m/s)
...
void SetGameVolume(int volume)
{
	...
	if(theDefaultSoundOutputDevice){
		...
		if(gSystemVolumeActive)
		{
			...
			DoError(GetSoundOutputInfo(theDefaultSoundOutputDevice,siSampleRate,&gSystemRate));
		}
		DoError(GetSoundOutputInfo(theDefaultSoundOutputDevice,siSampleRateAvailable,&rates));
		for(i=0;i<rates.count;i++)
			if(((UnsignedFixed*)*rates.infoHandle)[i]==searchRate)
				found=true;
		if(found) 
			DoError(SetSoundOutputInfo(theDefaultSoundOutputDevice,siSampleRate,(void*)searchRate));
...
void SetSystemVolume()
{
	...
	if(theDefaultSoundOutputDevice&&gSystemRate){
		...
		DoError(SetSoundOutputInfo(theDefaultSoundOutputDevice,siSampleRate,(void*)gSystemRate));
	}
}
...
void InitChannels()
{
	...
	gBuggySoundManager=U32Version(SndSoundManagerVersion())<0x03600000&&U32Version(SndSoundManagerVersion())>=0x03300000;
}
...
void SetCarSound(float engine,float skidL,float skidR,float velo)
{
	if(gPrefs.engineSound&&gPrefs.sound){
		...
		if(!(*gRoadInfo).water){
			...
			while(velo>(gGear+1)*highestGear/kNumGears+kShiftTolerance&&(gGear+1)<kNumGears)
				gGear++;
			while(velo<gGear*highestGear/kNumGears-kShiftTolerance&&gGear>0)
				gGear--;
		...
		velo=fabs(velo);
		if(engine!=-1)
			engineVol=-gVolume/((0.6*engine+0.15*velo+0.25*gearVelo)-2);	
		else
			engineVol=0;
		...
		cmd.cmd=volumeCmd;
		cmd.param1=0;
		cmd.param2=((int)(0x0048*engineVol*gVolume)&0xffff)|((int)(0x0048*engineVol*gVolume)<<16);
		DoError(SndDoImmediate(gEngineChannel,&cmd));
		if(gBuggySoundManager){
			cmd.cmd=rateCmd;
			cmd.param2=0x00016000*(0.2+0.3*engine+0.2*velo+0.3*gearVelo);
			DoError(SndDoImmediate(gEngineChannel,&cmd));
		}
		else{
			cmd.cmd=rateMultiplierCmd;
			cmd.param2=0x0000b000*(0.2+0.3*engine+0.2*velo+0.3*gearVelo);
			DoError(SndDoImmediate(gEngineChannel,&cmd));
		}
		...
		cmd.cmd=volumeCmd;
		cmd.param1=0;
		cmd.param2=((int)(0x0100*(skidL*gVolume*(velo*0.5+0.5)))&0xffff)|((int)(0x0100*(skidR*gVolume*(velo*0.5+0.5)))<<16);
		DoError(SndDoImmediate(gSkidChannel,&cmd));
		if(gBuggySoundManager){
			cmd.cmd=rateCmd;
			if(skidL+skidR)
				cmd.param2=0x00010000*(-1/((skidL+skidR)*0.25+velo*0.5-2));
			else
				cmd.param2=0;
			DoError(SndDoImmediate(gSkidChannel,&cmd));
		}
			...
			cmd.cmd=rateMultiplierCmd;
			if(skidL+skidR)
				cmd.param2=0x00020000*(-1/((skidL+skidR)*0.25+velo*0.5-2));
			else
				cmd.param2=0;
			DoError(SndDoImmediate(gSkidChannel,&cmd));
		}
